...
function saveOrUpdateProduct() {
    ...
    $('#product_edit_dialog').dialog("close",true);
...
function saveOrUpdateProductCategory() {
    ...
    // });
    $('#edit_product_category_form').form('submit', {
        url: ctx + "/product/category/saveOrUpdateProductCategory",
        onSubmit: function () {
            $.messager.progress({
                title: '提示',
                text: '数据处理中，请稍后....'
        ...
        success: function (data) {
            ...
            if (data.statusCode == 0) {
                ...
                setTimeout(
                    function () {
                        $('#deal_category_edit_dialog').dialog("close",true);
                        $("#product_category_tree").tree("reload");
